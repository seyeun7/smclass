...
# 화면을 스크롤해서 내리기 반복
# excute_script() : 자바스크립트 실행
prev_height = browser.execute_script("return document.body.scrollHeight")

while True:
    browser.execute_script("window.scrollTo(0,document.body.scrollHeight)")
    time.sleep(1)
    # 페애ㅣ지가 로딩되면서 길어진 길이를 다시 가져옴.
...
# 기준점
data = soup.select_one("#__next > div > main > section > div.PlaceListBodyTopPadding_searchBaseNavArea__eSY7d")
# 호텔 여러개 정보 리스트
...
true_num = 0   # 맞는 개수
fail_num = 0   # 맞지 않는 개수
except_num = 0 # 예외처림 - 마감(금액없을시)
search_list = []

for idx,list in enumerate(lists):
    search = []
    try:
        rating  = float(list.select_one("span.PlaceListScore_rating__3Glxf").text)
        price = int(list.select_one("span.PlacePriceInfoV2_discountPrice__1PuwK").text)
        if rating<4.8 or price>180000:
            price(f"{idx+1}. 조건에 맞지 않음.")
            fail_num += 1
            continue
        print(f"{idx+1}.")
        name = list.select_one("PlaceListTitle_container__qe7XH").text
        print("호텔명 :",name)
        print("평점 :",rating)
        print("금액 :",price)
        print("-"*50)
        search.append([idx+1,name,rating,price])
        true_num += 1
...
# 금액 순차정렬
search_list.sort(key=lambda x:x[4])
print(search_list[:3])
